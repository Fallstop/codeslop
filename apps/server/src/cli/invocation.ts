 ...
 * Detection is best-effort; callers must fail closed to a plain `slop` command.
 */
...
/**
 * Render a `slop <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx t3 connect` suggests
 * `npx t3 serve` (the runner needs the package spec, not the `slop` bin name),
 * a global install suggests `slop serve`, and a nightly build keeps the
 * `@nightly` tag.
 */
