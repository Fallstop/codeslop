...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `slop connect link` to authorize and enable T3 Connect."
    : !status.desired
      ? "Run `slop connect link` to enable T3 Connect."
      : !status.linked
        ? "Start codeslop to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.Confirm({
      message: `The codeslop relay client is required for T3 Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
    ...
    if (Exit.isFailure(input.relayResult)) {
      ...
      yield* Console.warn(
        input.clearAuthorization
          ...
          : "Could not revoke the relay-side environment record yet.\nRun `slop connect unlink` again when the relay is reachable.",
      );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (options.clearAuthorization) {
    yield* Console.log(
      "Signed out of T3 Connect locally.\nThe background service is managed separately with `slop service`.",
    );
  }
});
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start codeslop to publish agent activity (no managed tunnel).`
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!(yield* tokens.hasCredential)) {
          yield* Console.log(
            "Run `slop connect login` first so this environment can be authorized to publish.",
          );
          return;
        }
        // A link may already be desired (e.g. `slop connect link` before the
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A T3 Connect link is already pending. Start codeslop to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart codeslop to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (background) {
          ...
          yield* Console.log(
            platform === "darwin"
              ? "\n✓ Background service ready\n\ncodeslop is set to run while you are logged in to this Mac. The server establishes the T3 Connect link on startup."
              : "\n✓ Background service ready\n\ncodeslop is set to keep running after you log out. The server establishes the T3 Connect link on startup.",
          );
          return;
        }
        const serveCommand = yield* resolveCliCommand("serve");
