...
export function decodePullRequestCoreJson(
  ...
  return Result.succeed({
    ...
    viewerAccess: {
      ...
      ...defaultMergeMethodFields(repository.viewerDefaultMergeMethod),
    },
    comparison:
...
/** Omitted rather than null when GitHub does not answer, so the optional field stays absent. */
function defaultMergeMethodFields(viewerDefaultMergeMethod: string | null | undefined): {
  defaultMergeMethod?: PullRequestMergeMethod;
} {
  const defaultMergeMethod = toDefaultMergeMethod(viewerDefaultMergeMethod);
  return defaultMergeMethod === undefined ? {} : { defaultMergeMethod };
}
