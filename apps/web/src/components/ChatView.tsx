...
export default function ChatView(props: ChatViewProps) {
  ...
  const composerBannerItems = useMemo<ComposerBannerStackItem[]>(() => {
    // Urgent notices lead; the calm ones sit behind the handoff banner, which
    // changes what the user can do rather than just what they should know.
  ...
  const onSend = async (
    ...
    const runtimeModeForSend = runtimeMode;
    const interactionModeForSend = sendInteractionMode;
    ...
    // text. A draining queued message is exempt — it goes to the thread it was
    // stacked on, not into a side chat.
    const sideChatCommand = queuedMessage ? null : parseSideChatCommand(trimmed);
    if (sideChatCommand) {
      promptRef.current = "";
      clearComposerDraftContent(composerDraftTarget);
      composerRef.current?.resetCursorState();
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's codeslop server to browse pull requests."
      />
      ...
      />
    ) : renderedRightPanelSurface?.kind === "side-chats" && activeThreadRef && activeThread ? (
      <SideChatPanel
        environmentId={activeThread.environmentId}
