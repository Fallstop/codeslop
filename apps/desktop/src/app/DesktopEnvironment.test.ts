...
describe("DesktopEnvironment", () => {
  ...
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "app.codeslop.desktop.dev");
      assert.equal(environment.linuxWmClass, "codeslop-dev");
      assert.equal(environment.linuxDesktopEntryName, "codeslop-dev.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "codeslop.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.codeslop/dev");
      assert.equal(production.stateDir, "/Users/alice/.codeslop/userdata");
    }),
  );
