...
describe("electron development launcher", () => {
  ...
  it("keeps the native Electron executable name inside the branded macOS bundle", () => {
    const paths = resolveMacLauncherPaths(
      "/repo/apps/desktop/.electron-runtime/codeslop (Dev).app",
      "codeslop (Dev)",
    );

    assert.equal(paths.launcherExecutableName, "codeslop (Dev) Launcher");
    assert.equal(
      paths.launcherBinaryPath,
      "/repo/apps/desktop/.electron-runtime/codeslop (Dev).app/Contents/MacOS/codeslop (Dev) Launcher",
    );
    assert.equal(
      paths.runtimeElectronBinaryPath,
      "/repo/apps/desktop/.electron-runtime/codeslop (Dev).app/Contents/MacOS/Electron",
    );
    ...
    assert.include(
      script,
      "exec '/repo/apps/desktop/.electron-runtime/codeslop (Dev).app/Contents/MacOS/Electron'",
    );
  ...
  it("derives launcher icons from canonical development and production assets", () => {
    ...
    assert.match(production.sourceIconPath, /assets[\\/]prod[\\/]codeslop-macos-1024\.png$/);
