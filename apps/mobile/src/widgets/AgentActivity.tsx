...
export function AgentActivity(
  ...
  // The branded codeslop mark. `assetName` resolves the template image set
  // bundled in the widget extension's asset catalog. Image views only honor
  // `resizable` directly (frame/foregroundStyle are dropped), so we size it via
  // a container frame the resizable image fills and tint it through the
  // container's foreground style, which the template image inherits. The 1.2:1
  // frame matches the glyph's aspect ratio so it never distorts.
  const renderLogo = (height: number, color: string) => (
    <HStack modifiers={[frame({ width: height * 1.2, height }), foregroundStyle(color)]}>
