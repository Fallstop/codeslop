...
const DESKTOP_APP_ID = "app.codeslop.desktop";
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // codeslop always passes the user's installed Claude executable to the SDK,
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "codeslop (Nightly)"
    : (desktopPackageJson.productName ?? "codeslop");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    files: [
      ...
      ...resolveOnnxRuntimeExclusions(platform),
    ],
    directories: {
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "codeslop",
          schemes: ["codeslop", "codeslop-dev", "t3code", "t3code-dev"],
        },
      ],
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    // Electron derives the default userData dir from this name before the in-app override
    // runs, so it must match DesktopEnvironment's `codeslop` naming.
    name: "codeslop",
    version: appVersion,
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "codeslop desktop build",
    author: "codeslop contributors",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for codeslop."),
