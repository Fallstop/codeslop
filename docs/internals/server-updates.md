...
The launcher is the only runtime writer of `service-state.json`. `slop service install` and
`slop service update` may replace the launcher and state while the unit is stopped. Server children
