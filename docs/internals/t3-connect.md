...
> For maintainers. Using codeslop? See [docs/user](../user/).
...
When any client-facing public value is absent, cloud UI is omitted. The `slop connect` command group is
...
The `slop connect` commands authorize a headless environment with a separate Clerk OAuth application.
...
```sh
slop connect            # default: onboarding
slop connect login
slop connect link       # --publish-only
slop connect status     # --json
slop connect publish    # --disable
slop connect unlink
slop connect logout
```

`slop serve` is a separate top-level command, not a connect subcommand.

`slop connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `slop connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running T3 server. The next `slop serve` or `slop start` reconciles the relay link and launches the
managed tunnel. `slop connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `slop connect link` can re-enable exposure without another browser flow. `slop connect
...
logout leaves it running; manage it with `slop service status`, `install`, `update`, and `uninstall`.
...
T3CODE_PORT=13773 \
  "/Applications/codeslop (Alpha).app/Contents/MacOS/codeslop (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/codeslop (Alpha).app"
codesign -d --entitlements :- "/Applications/codeslop (Alpha).app"
```
