...
This guide is for people who want to use more than one Claude setup in codeslop. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install codeslop](./install.md).
...
In codeslop Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means codeslop uses Claude Code's normal config directory.

When you set this field, codeslop points Claude Code at that directory with the
...
codeslop looks for Claude skills in the Claude config directory's `skills` folder, then
...
`~/.claude_personal_home/.claude`, which is not where codeslop looks.

Then add another Claude provider in codeslop:
...
codeslop only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so codeslop keeps separate config directories isolated
...
Add or edit a Claude provider in codeslop Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. codeslop stores the value as a server secret and does not
...
codeslop does not need a special Claude Code Router provider. Treat the router as a Claude
