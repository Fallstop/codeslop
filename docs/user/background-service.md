# Running codeslop in the Background

On a Linux host, codeslop can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest codeslop release:
...
Updating restarts codeslop briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The systemd unit runs a small stable launcher. Exact codeslop versions are installed separately, so
...
Signing out of T3 Connect does not remove the service. Use `slop service uninstall` when you no longer
want codeslop to start in the background.
