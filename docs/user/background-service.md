...
On Linux and macOS, codeslop can run as a service for your user so you do not need
...
Linux needs systemd user services. Setup enables lingering so codeslop starts at
...
Start with `slop service status` on the host. It prints the log path and, on Linux,
...
with sudo; running codeslop as root creates a separate installation and Connect
identity. Without administrator access, run `slop serve` in a terminal and keep
that session open.
