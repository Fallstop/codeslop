...
On Linux and macOS, codeslop can run as a background service for your user, so it is ready without
...
The service runs a small stable launcher. Exact codeslop versions are installed separately, so a
...
Signing out of T3 Connect does not remove the service. Use `slop service uninstall` when you no
longer want codeslop to start in the background.
