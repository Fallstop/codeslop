...
codeslop runs coding agents on your computer and lets you control them from its
...
launch codeslop and configure providers afterwards.
...
there. Install Node.js and provider CLIs inside that distro. codeslop installs its
...
Pass a path, such as `npx slop app ../my-project`, to open another directory. It requires
the desktop app, so a standalone server or an SSH session is not enough. If the
command cannot reach the app, start or update the desktop app and try again.

## Mobile app

Install codeslop from the
...
| Antigravity | Install and sign in with Google from codeslop's provider settings.                           |

Provider CLIs must be on the server's `PATH`. If codeslop cannot find one, set its
**Binary path** in provider settings, especially when using a version manager.
Cursor's executable is `cursor-agent`, although its login command is
`agent login`. Antigravity can use its managed runtime without a `PATH` entry.

When a provider CLI is behind its latest release, its provider card shows the
available version. **Update now** appears only when codeslop can tell which
...
base URL. Mark secret values as sensitive; after saving, codeslop does not display
...
- [Updating codeslop](./updating.md): update the app and connected servers.
