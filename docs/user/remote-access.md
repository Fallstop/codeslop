...
Connect a phone, browser, or another desktop app to codeslop running on a different
...
server with `npx slop serve`. Saving your sign-in alone does not make the machine
reachable.
...
```bash
npx slop serve --host <private-ip>
```
...
```bash
npx slop serve --tailscale-serve
```

For an already-running server:

```bash
npx slop pair --tailscale
```
...
`--tailscale-serve-port`. See `npx slop pair --help` for other pairing options.
...
**SSH**, and enter a host or SSH alias such as `user@example.com`. codeslop starts
...
The first launch downloads codeslop's server to `~/.t3/runtime` on the host, so
...
the connection stops a server that codeslop launched; a server that was already
...
management is available through `npx slop auth --help`.
...
On a command-line host, `slop connect unlink` disables exposure while retaining
your login; `slop connect logout` also clears that login. Background-service
...
Run `slop connect status` on the host to inspect saved authorization and link
configuration. It is not a live reachability check. If the environment appears
offline, run `slop service status` and read the displayed log. If it disappears
when SSH closes, see [background-service troubleshooting](./background-service.md#troubleshooting).

| Error                                                     | Recovery                                                                                                                                          |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `environment_link_limit_exceeded` or managed tunnel limit | Deregister an unused environment, then restart codeslop on the host.                                                                              |
| `auth_invalid` or `invalid_bearer`                        | Run `slop connect login`. If credentials were revoked, run `slop connect logout`, then `slop connect` again. Restart the server after signing in. |
| Expired or invalid link proof                             | Check the host's date and time, update codeslop, then restart it.                                                                                 |
...
foreground server, stop it and run `slop serve` again with your usual options.
...
devices. For server version warnings, follow [Updating codeslop](./updating.md).
...
environment**. codeslop restarts without a local server: no local agents or terminals run, WSL
