...
Use this when you want to connect to a codeslop server from another device such as a phone, tablet, or separate desktop app.
...
`slop pair` finds the running server (the shared `~/.t3` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
If no server is running, `slop pair` says so and points you at `npx t3 serve` or `npx t3 connect`.
...
run a headless server from the CLI, or have the desktop app launch codeslop over SSH.
...
backend with the same server-side behavior as `slop serve --tailscale-serve`, then the server asks
...
Run the server with `slop serve`.

```bash
npx t3 serve --host "$(tailscale ip -4)"
```

`slop serve` starts the server without opening a browser and prints:
...
Use `slop serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.
...
Use this when you want the desktop app to start or reuse codeslop on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. codeslop uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, codeslop first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path codeslop uses:
...
When the codeslop web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: codeslop may
...
see [Keeping codeslop in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running codeslop in the Background](./background-service.md).
...
1. `slop serve` issues a one-time owner pairing token.
...
Hosted pairing does not proxy traffic through codeslop. The browser still connects directly to the backend URL in the pairing link.

## Managing Access Later

Use `slop auth` to manage access after the initial pairing flow.
...
Use `slop auth --help` and the nested subcommand help pages for the full reference.
...
- Use `slop auth` to revoke credentials or sessions you no longer trust.
