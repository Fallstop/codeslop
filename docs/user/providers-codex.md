...
This guide is for people who want to use more than one Codex account in codeslop. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install codeslop](./install.md).
...
In codeslop Settings, name it something obvious:
...
In codeslop Settings, add another Codex provider:
...
codeslop shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, codeslop treats it
